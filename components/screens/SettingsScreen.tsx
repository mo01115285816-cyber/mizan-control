    ...
    <div className="space-y-7 animate-in fade-in duration-200">
      ...
      <form onSubmit={handleSaveSettings} className="space-y-5">
        ...
        <section className="bg-[#FFFDF8] border border-[#E3E5DC] rounded-[28px] p-5 sm:p-7 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3"><input value={inviteName} onChange={(e) => setInviteName(e.target.value)} className={inputClass} placeholder="اسم العضو اختياري" /><button type="button" onClick={() => void createInvite(inviteName)} className="px-5 py-3 rounded-full bg-[#151515] text-[#FFFDF8] font-bold text-sm whitespace-nowrap">إنشاء دعوة حقيقية</button></div>
          {activeInvite && <div className="space-y-3 bg-[#E7F5C8] border border-[#C8F24A] rounded-2xl p-4"><p className="text-xs font-bold">الرابط الذي يمكن فتحه على الهاتف</p><div className="flex items-center gap-2"><code className="flex-1 text-[11px] break-all dir-ltr">{activeInvite.deepLink}</code><button type="button" onClick={() => void copyText(activeInvite.deepLink)} className="p-2 rounded-full bg-[#FFFDF8]" title="نسخ"><Copy className="w-4 h-4" /></button></div><p className="text-[11px] text-[#777A72]">يمكن أيضًا استخدام الرابط العام: {activeInvite.webLink}</p></div>}
        </section>
